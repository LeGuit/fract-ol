...
int					mouse_zoom_in(t_data *data, int x, int y)
{
	...
	zoom.pr = data->bl.pr + (double)x * data->delta.pr;
	zoom.pi = data->bl.pi +
			((double)data->mlx->screen.height - (double)y) * data->delta.pi;
	data->delta.pr /= 1.1f;
	data->delta.pi /= 1.1f;
	data->bl.pr = zoom.pr - (double)x * data->delta.pr;
	data->bl.pi = zoom.pi -
			((double)data->mlx->screen.height - (double)y) * data->delta.pi;
	return (0);
}
...
int					mouse_zoom_out(t_data *data, int x, int y)
{
	...
	zoom.pr = data->bl.pr +
			((double)data->mlx->screen.width - (double)x) * data->delta.pr;
	zoom.pi = data->bl.pi + (double)y * data->delta.pi;
	data->delta.pr *= 1.1f;
	data->delta.pi *= 1.1f;
	data->bl.pr = zoom.pr -
			((double)data->mlx->screen.width - (double)x) * data->delta.pr;
	data->bl.pi = zoom.pi - (double)y * data->delta.pi;
	return (0);
}
